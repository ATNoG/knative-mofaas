...
class MoFaaSProxy:
    ...
    async def proxy_handler(self, request):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for url in chosen_funcs:
                ...
                tasks.append(asyncio.create_task(self.__do_request(session=session, method=method, url=target_url, headers=headers, body=body)))
            ...
            for i1 in range(len(responses)):
                for i2 in range(len(responses)):
                    ...
                    for k in differences_matrixes:
                        await self.__fill_resultant_structs_defaults(differences_matrixes, responses, key=k, index1=i1, index2=i2)
                        ...
                        if responses[i1][k] != responses[i2][k]:
                            if k != 'headers':
                                ...
                                await self.__fill_resultant_structs(differences_matrixes, equal_results, responses, key=k, index1=i1, index2=i2)
                            else:
                                for header in list(responses[i1][k].keys()) + list(responses[i2][k].keys()):
                                    await self.__fill_resultant_structs_defaults(differences_matrixes, responses, key=k, index1=i1, index2=i2, header=header)
                                    ...
                                    if header not in responses[i1][k] and header in responses[i2][k]:
                                        ...
                                        await self.__verify_ignore_headers(differences_matrixes, equal_results, responses, key=k, index1=i1, index2=i2, header=header, message=message)
    ...
    async def __do_request(self, session: aiohttp.ClientSession, method, url, headers, body):
    ...
    async def __fill_resultant_structs_defaults(self, differences_matrixes, responses, key, index1, index2, header=None):
        if not header:
    ...
    async def __fill_resultant_structs(self, differences_matrixes, equal_results, responses, key, index1, index2, header=None):
        if header:
    ...
    async def __verify_ignore_headers(self, differences_matrixes, equal_results, responses, key, index1, index2, header, message):
        if header in self.ignore_headers:
            ...
            await self.__fill_resultant_structs(differences_matrixes, equal_results, responses, key=key, index1=index1, index2=index2, header=header)
